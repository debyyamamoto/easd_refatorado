...
class EASD:
    def __init__(self, data : pd.DataFrame, time_col:str, event_col:str, crossover_rate, max_generations, mutation_rate,
                 population_size, restart_check_point, restart_percentage, seed_val, comparacao:str, alpha, executions, ksize):
    ...
    def run(self):
            ...
            print(f"\n{'='*70}")
            print(f"--- Iniciando Busca Top-K ({self.executions} execuções) ---")
            print(f"\n{'='*70}")
            print(f"Configuração:")
            print(f"   • População: {self.population_size}")
            print(f"   • Gerações: {self.max_generations}")
            print(f"   • Execuções: {self.executions}")
            print(f"   • Top-K: {self.ksize} melhores regras")
            print(f"   • Crossover: {self.crossover_rate}% | Mutação: {self.mutation_rate}%")
            print(f"{'='*70}")
            ...
            for i in range(self.executions): 
                ...
                while gen_count < self.max_generations:
                    ...
                    if fitness_list:
                        ...
                        if (gen_count + 1) % 50 == 0:
                            print(f"   Gen {gen_count + 1:3d}: Melhor={best_fit:.4f} | "
                                f"Média={mean_fit:.4f} | Top-K={len(self.best_by_key)}/{self.ksize}")
                    else:
                        print(f"⚠️  AVISO G{gen_count}: População vazia - Encerrando execução.")
                ...
                if gen_best_fitness:
                    ...
                    print(f"   ✓ Concluída: {gen_count} gerações | "
                        f"Melhor Fitness Final={final_best:.4f}\n")
            print(f"\n{'='*70}")
            print(f" Finalizou {self.executions} execuções em {time.time() - start_time:.2f}s")
            print(f"{'='*70}")           
            ...
            if top_k_values:
                ...
                print(f"   • Média Fitness: {avg_fit:.4f} (±{std_fit:.4f})")
                print(f"   • Melhor: {best_fit:.4f}")
                print(f"   • Pior: {worst_fit:.4f}")
            else:
                print("\n Nenhuma regra válida encontrada!")
            ...
            for fit, rule_raw in sorted_rules:
                rule_adjusted = self.adjust_interval(copy.deepcopy(rule_raw), dataset_x)
            ...
            print(f"\n Resultados Finais:")
            print(f"   • Regras encontradas: {rules_qtd}")
            print(f"   • Tamanho médio: {mean_size:.2f} atributos")
            print(f"   • Tempo total: {total_time:.2f}s")
            print(f"{'='*70}\n")
            detailed_rules_df = pd.DataFrame({'Rule_Obj': [str(r) for r in final_rules_found]})
